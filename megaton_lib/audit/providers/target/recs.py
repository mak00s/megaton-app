...
def export_recs(
    ...
    *,
    ...
    prune : delete local files that no longer exist in the API after export.
        Only safe with a full unfiltered export; raises ValueError if combined
        with name_regex, id_list, or max_items.  Targets files generated by
        this function (flat layout).  Do not use on directories containing
        manually maintained sidecars such as code/ subdirectories.
    ...
    for resource in resources:
        ...
        if prune and _has_export_filters(name_regex=nr, id_list=il, max_items=mi):
            raise ValueError(
                "Refusing Target export prune with filters. "
                "Run a full unfiltered export, or disable prune.",
            )

        count = _export_resource(
            client,
            resource,
            res_dir,
            name_regex=nr,
    ...
    *,
    ...
    for item in items:
        ...
        if resource == "designs" and isinstance(detail, dict):
            sidecar = _extract_design_script(detail, out_dir, item_id)
            if sidecar:
    ...
    if prune:
        _prune_export_dir(out_dir, resource, expected_files)
    return len(items)
...
def _extract_design_script(design: dict[str, Any], out_dir: Path, item_id: Any) -> str | None:
...
def _has_export_filters(
    *,
    ...
    """Return True when export is filtered and therefore unsafe to prune."""
    return bool(name_regex or id_list or max_items is not None)


def _prune_export_dir(out_dir: Path, resource: str, expected_files: set[str]) -> None:
    """Delete stale exported files for a full unfiltered resource export."""
    allowed_extensions = {".json"}
    if resource == "designs":
        allowed_extensions.update({".html", ".js", ".vtl"})
    ...
    for existing in out_dir.iterdir():
        ...
        if existing.is_dir():
            ...
            continue
        if existing.suffix not in allowed_extensions:
            print(f"[prune] {existing.name}")
            existing.unlink()
            continue
