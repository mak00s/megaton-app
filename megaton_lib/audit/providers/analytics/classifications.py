...
class ClassificationsClient:
    ...
    def _resolve_classification_dim(
        ...
        *,
        ...
        Adobe Analytics Reporting API exposes each classification as a
        separate dimension with an arbitrary slug, not as
        ``variables/{dimension}.{N}`` based on column order. The slug is
        chosen by whoever created the classification (e.g.
        ``stepd-202404202603``) and may be Pinyin, abbreviations, or
        autogenerated. We look it up by querying the ``/dimensions``
        endpoint and matching the human-readable column name.

        Falls back to the legacy numeric index when the ``/dimensions``
        endpoint is not reachable, but that index has been observed to
        return data for an unrelated classification, so callers should
        prefer the slug path whenever ``rsid`` is available.

        Returns
        -------
        str
            e.g. ``"variables/evar30.stepd-202404202603"``
        """
        # 1. Verify the column exists in the classification table.
        columns = self.get_classification_columns(dataset_id, verbose=False)
        if column not in columns:
            raise RuntimeError(
        ...
        dim_base = (
            dimension
            if dimension.startswith("variables/")
            else f"variables/{dimension}"
        )

        # 2. If we know the rsid, fetch all dimensions and find the slug
        #    whose human-readable name / title matches the column.
        if rsid:
            try:
                url = (
                    f"{AA_API_BASE}/{self.company_id}/dimensions"
                    f"?rsid={rsid}"
                )
                ...
                for dim in resp.json():
                    ...
                    if not dim_id.startswith(dim_base + "."):
                        continue
        ...
        #    paths without rsid continue to function.
    ...
    def verify_column_via_report(
        ...
        *,
        ...
        classification_dim = self._resolve_classification_dim(
            dataset_id, column, dimension, rsid=rsid,
        )
        if verbose:
            print(f"[info] Dataset: {dataset_id} ({rsid} / {dimension})")
