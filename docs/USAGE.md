...
- チャートは `X軸` / `Y軸` / `系列（任意）` を選択可能
  - `date` など datetime 系列がある場合は `X軸` に自動でデフォルト選択
  - `系列（任意）` にカテゴリ列を指定すると、2つめディメンションで系列分割
- CSV 保存 / ダウンロード
- 日英切り替え

### 実行ボタンの挙動

- 「実行」を押すとボタン表示が「実行中...」に変わり、処理完了まで再クリック不可
- 実行中はサイドバーに取得中メッセージを表示
