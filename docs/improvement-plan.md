...
> **📁 アーカイブ (2026-08-10)**: 2026年6〜7月の改善プロジェクトの完了記録。
> 以後の変更履歴の正本は各repoの CHANGELOG(megaton: CHANGELOG.md / app: docs/CHANGELOG.md)と
> リリースタグに移行済み。このファイルは歴史的経緯・設計判断の参照用として保持。
>
> 2026-08-10 の3repo連携監査で全消費repoのタグピン運用・GHA全グリーンを確認。
> 監査で発見した Sheets retry ドリフト(quota-403)は megaton v2.0.1 で解消し、
> notebooks のピンを megaton v2.0.1 / megaton-app v0.28.0 に更新済み。

最終更新: 2026-06-12 / 状態: **プロジェクト完了** 🎉
Step 1〜7・実走検証・デプロイ(PyPI 1.4.1)・report_run横展開・docs同期、すべて完了。
...
2. [x] **slqm.py 実走検証 ✅ 完了 (2026-06-11 夜)**: `_page`/`_page-d`/`_page-m`/`_all-m` は本番と**全セル一致**、`_info` は News一致+Voiceのみ新方式が精密(GA4カーディナリティ丸めの解消、ユーザー承認済みで採用)。詳細エビデンスは notebooks/reports/slqm.md「検証状況」に記録。**後始末1件: 一時コピー `_tmp_slqm_verify_20260611` はSA権限不足で削除不可 → WITH Report共有ドライブからユーザーが手動削除**。比較スクリプトは削除済み
   ...
   - **megaton v1.4.1 で修正・PyPI公開済み**: `report.run(timeout=180)` デフォルト追加(run_reportへpass-through)。
