...
```
    ...
    ┌────────┐          ┌────────┐          ┌────────┐
    │ Note   │          │  CLI   │          │   UI   │
    │ book   │          │ script │          │Streamlit
    │        │          │        │          │        │
    │ .py    │──CLI実行→│query.py│          │  :8501 │
    │(jupytext)         │        │          │        │
    └────────┘          └────────┘          └────────┘
...
AI Agent と人間が協力してデータ分析を行う際、それぞれの方式には課題がある：

**❌ Agent単独の課題:**
- パラメータの妥当性を人間が確認できない
- 日付範囲やフィルタ条件のミスに気づけない
- 実行前に「本当にこれで合ってる？」の確認ができない

**❌ 人間単独の課題:**
- SQLやAPIパラメータを毎回書くのが面倒
- 複雑なフィルタ構文（`sessionDefaultChannelGroup==Organic Search;country==Japan`）を覚える必要がある
- JSON形式でパラメータを書くのは手間

**✅ Streamlit UIによる解決:**

| ステップ | 人間の役割 | AI Agentの役割 |
|---------|-----------|---------------|
| 1. 要求 | 自然言語で指示<br>「直近7日間のOrganic Searchの推移」 | - |
| 2. 生成 | - | JSONパラメータを自動生成<br>`input/params.json` に書き込み |
| 3. 確認 | UIで自動反映された内容を確認<br>日付・フィルタを微調整 | - |
| 4. 実行 | ボタン押下（or 自動実行） | - |
| 5. 検証 | 結果を目視確認<br>OKなら次の分析へ | `output/*.csv` を読んで分析続行 |

**Streamlitを選んだ技術的理由:**
- サイドバーでパラメータ、メインで結果という自然な構成
- データフレーム表示・チャート描画が標準機能
- `@st.cache_data` で再実行が高速
- ファイル監視（2秒ごと）でAI Agentとのリアルタイム連携
